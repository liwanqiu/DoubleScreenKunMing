...
public class BusInfoParser {
    ...
    public BusInfoParser(Context context) {
        ...
        parseThread.setRunning(true);
        parseThread.start();
    }

    public void stop() {
        parseThread.setRunning(false);
    }
    ...
    public void read(byte[] bytes) {
        parseThread.hasNewData = true;
        parseThread.setDatas(bytes);
    }
        ...
        private void parse(byte[] data) {
            ...
            while (true) {
                ...
                if (lineName != null && departureTime != null && busCustomiseNum != null) {
                    ...
                    //单位由毫秒转为秒
                    final long time = departureTime * 1000;
                    final String name = lineName;
                    final String num = busCustomiseNum;
                    if (onBusInfoListener != null) {
                        ((BaseActivity) context).runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                onBusInfoListener.onReceived(new BusInfo(time, name, num));
                            }
                        });
                    }
                    break;
